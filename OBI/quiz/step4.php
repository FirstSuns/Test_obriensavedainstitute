...
// load record from 'programs_overview'
  ...
  $blogRecord = @$blogRecords[0]; 
	...
	if(isset($_POST['btn_step3'])) {
		unset($_POST['btn_step3']);
		if(isset($_POST['spend_time_val']) && !empty($_POST['spend_time_val']) && is_numeric($_POST['spend_time_val']) && $_POST['spend_time_val'] >0 && (isset($_POST['spend_time_type']) && !empty($_POST['spend_time_type']))) {
			$_SESSION['sess_step3'] = $_POST;
	...
	<script>
		$(document).ready(function(){
			...
			$(".quizitem").click(function(){
				...
				$(".c-celebrity_style_val").val(val);
				$(".c-celebrity_style_type").val(type);
			});
			
			<?php
			if(isset($_SESSION['sess_step4']['celebrity_style_type'])) {
				?>
				is_selected = true;
				<?php
			}
...
<div style="clear: both;"></div>
		...
		<div class="quizsub">
			<div class="wrapper">
			<h1>Step 3 of 4</h1>
			WHICH CELEBRITY WOULD YOU LET BE YOUR STYLIST?
			...
			foreach($arrDivs as $index => $val )	{
				$new_cls = '';
				if(isset($_SESSION['sess_step4']['celebrity_style_type']) && $_SESSION['sess_step4']['celebrity_style_type'] ==$val['type']) {
			...
			<form method="post" id="idfrm4" action="step5.php">
				<input type="hidden" name="celebrity_style_val" value="<?=$_sele_val?>" class="c-celebrity_style_val" />
				<input type="hidden" name="celebrity_style_type" value="<?=$_sele_type?>" class="c-celebrity_style_type" />
				<input type="hidden" name="btn_step4" class="quizbutton2 c-opacity" value="NEXT" /> 
			</form>
