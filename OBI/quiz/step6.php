...
// load record from 'programs_overview'
  ...
  $blogRecord = @$blogRecords[0]; 
	...
	/* bg images */
	$arrFinalImg['Cosmetology'] = array(
		'Fashion Photo Stylist'		=> "cosmetology-fashion-stylist.jpg",
		'Master Hair Colorist' 		=> "cosmetology-master-hair-colorist.jpg",
		'Salon Owner'				=> "cosmetology-salon-owner.jpg"
	);
	
	$arrFinalImg['Barbering'] = array(
		'Neighborhood Barber'	=> "barbering-neighborhood-barber.jpg",
		'Barbering Educator'	=> "barbering-barbering-educator.jpg",
		'Barber Shop Owner'		=> "barbering-barbershop-wner.jpg"
	);
	
	$arrFinalImg['Esthetics'] = array(
		'Skin Care Specialist'	=> "esthetics-skin-care-specialist.jpg",
		'Master Esthetician'	=> "esthetics-master-esthetician.jpg",
		'Resort Spa Manager'	=> "esthetics-resort-spa-manager.jpg"
	);
	...
	if(isset($_SESSION['sess_step5'])) {
		...
		$bg_img_show = $arrFinalImg[$select_type][$theShow_text];
		...
		*/
		...
		$headers= "From:O'Brien Aveda Institute<info@obriensavedainstitute.org>\r\n";
		...
		$query.=" `work_environment` = '".$select_type."', ";
		$query.=" `step_3` = '".$_SESSION['sess_step3']['spend_time_val']."', ";
		$query.=" `step_4` = '".$_SESSION['sess_step4']['celebrity_style_val']."', ";
		$query.=" `title_string` = '".$theShow_text."', ";
		$query.=" `show_message` = '".$string_to_show."' ";
		
		mysql_query($query) or die(mysql_error()." :$query:");
		
		
		$user_name = (isset($_SESSION['sess_step5']['firstname']) ? $_SESSION['sess_step5']['firstname'] : '').' '.(isset($_SESSION['sess_step5']['lastname']) ? $_SESSION['sess_step5']['lastname'] : '');
		/*
		$share_link = base64_encode($select_type).'-'.base64_encode($devided_val).'-'.base64_encode($theShow_text).'-'.$user_name;*/
		
		$share_link = $select_type.'-'.$devided_val.'-'.$theShow_text.'-'.$user_name;
		
		if(isset($_SESSION['sess_step2'])) {
			unset($_SESSION['sess_step2']);
		}
		if(isset($_SESSION['sess_step3'])) {
			unset($_SESSION['sess_step3']);
		}
		if(isset($_SESSION['sess_step4'])) {
		...
		echo '<meta http-equiv="refresh" content="0;url=step6.php?slug='.$share_link.'">';
		exit();
		
		
	} else {
		/*$_GET['slug'] = "Cosmetology-5-Salon Owner-dev dev";*/
		if(isset($_GET['slug']) && !empty($_GET['slug'])) {
			
			$arrT = explode("-",$_GET['slug']);
			
			$select_type = ucwords(strtolower($arrT[0]));
			
			$devided_val = $arrT[1];
			
			$theShow_text = ucwords(strtolower($arrT[2]));
			
			$user_name = ucwords(strtolower($arrT[3]));
...
<div class="quizsub" style="background-image: url(../images/<?=(isset($bg_img_show) ? $bg_img_show : 'celebbg.jpg')?>)">
<div class="wrapper">
<h5><?=$theShow_text?></h5>
<?=(isset($user_name) ? $user_name.',' : '')?><br />
<?=$string_to_show?>
