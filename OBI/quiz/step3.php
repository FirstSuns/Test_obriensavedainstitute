...
// load record from 'programs_overview'
  ...
  $blogRecord = @$blogRecords[0]; 
	...
	/* this set from step2.php file */
	if(isset($_POST['btn_step2'])) {
		unset($_POST['btn_step2']);
		if(isset($_POST['preferred_work_enviroment_val']) && !empty($_POST['preferred_work_enviroment_val']) && is_numeric($_POST['preferred_work_enviroment_val']) && $_POST['preferred_work_enviroment_val'] >0 && (isset($_POST['preferred_work_enviroment_type']) && !empty($_POST['preferred_work_enviroment_type']))) {
			$_SESSION['sess_step2'] = $_POST;
	...
	<script>
		$(document).ready(function(){
			...
			$(".quizitem").click(function(){
				...
				$(".c-spend_time_val").val(val);
				$(".c-spend_time_type").val(type);
			});
			
			<?php
			if(isset($_SESSION['sess_step3']['spend_time_type'])) {
				?>
				is_selected = true;
				<?php
			}
...
<div style="clear: both;"></div>
	...
	<div class="quizsub">
		<div class="wrapper">
			<h1>Step 2 of 4</h1>
			WHERE WOULD YOU LIKE TO SPEND YOUR OFF-TIME?
			...
			foreach($arrDivs as $index => $val )	{
				$new_cls = '';
				if(isset($_SESSION['sess_step3']['spend_time_type']) && $_SESSION['sess_step3']['spend_time_type'] ==$val['type']) {
			...
			<form method="post" id="idfrm3" action="step4.php">
				<input type="hidden" name="spend_time_val" value="<?=$_sele_val?>" class="c-spend_time_val" />
				<input type="hidden" name="spend_time_type" value="<?=$_sele_type?>" class="c-spend_time_type" />
				<input type="hidden" name="btn_step3" class="quizbutton2 c-opacity" value="NEXT" /> 
			</form>
			<div style="clear: both;"></div>
		</div>
	</div>
