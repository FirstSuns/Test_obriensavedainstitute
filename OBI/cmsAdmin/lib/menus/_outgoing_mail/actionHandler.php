...
function _ogm_showModeNotice($tableName, $action) {
  ...
  $notice = '';
  if     ($mode == 'sendOnly')   { $notice = t("Send Only - Send mail without keeping a copy"); }
  elseif ($mode == 'sendAndLog') { $notice = t("Send &amp; Log - Send mail and save copies under this menu"); }
  elseif ($mode == 'logOnly')    { $notice = t("Log Only - Log messages but don't send them (debug mode)"); }

  $notice = t("Outgoing Mail"). ": " . $notice . " (<a href='?menu=admin&action=general#email-settings'>" .t("change"). "</a>)";
  notice($notice);
}
